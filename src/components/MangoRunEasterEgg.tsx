...
const MANGO_SEQUENCE: MangoPhase[] = [
  { count: 4, mangoCount: 1, direction: "left", layout: [{ x: 1, y: 0.78, delay: 0, scale: 1 }] },
  {
    ...
    flipped: true,
    layout: [
      { x: 0.04, y: 0.7, delay: 0, scale: 1 },
      { x: -0.08, y: 0.84, delay: COUNT_DURATION / 4, scale: 1 },
    ],
  },
  {
    count: 4,
    ...
    direction: "down",
    layout: [
      { x: 0.12, y: 0, delay: 0, scale: 1 },
      { x: 0.37, y: 0, delay: COUNT_DURATION / 8, scale: 1 },
      { x: 0.63, y: 0, delay: COUNT_DURATION / 4, scale: 1 },
      { x: 0.86, y: 0, delay: (COUNT_DURATION * 3) / 8, scale: 1 },
    ],
  },
  {
    count: 4,
    ...
    direction: "up",
    layout: [
      { x: 0.06, y: 0, delay: 0, scale: 1 },
      { x: 0.19, y: 0, delay: COUNT_DURATION / 8, scale: 1 },
      { x: 0.32, y: 0, delay: COUNT_DURATION / 4, scale: 1 },
      { x: 0.46, y: 0, delay: (COUNT_DURATION * 3) / 8, scale: 1 },
      { x: 0.6, y: 0, delay: COUNT_DURATION / 2, scale: 1 },
      { x: 0.74, y: 0, delay: (COUNT_DURATION * 5) / 8, scale: 1 },
      { x: 0.88, y: 0, delay: (COUNT_DURATION * 3) / 4, scale: 1 },
      { x: 0.13, y: 0, delay: (COUNT_DURATION * 7) / 8, scale: 1 },
      { x: 0.27, y: 0, delay: COUNT_DURATION, scale: 1 },
      { x: 0.41, y: 0, delay: (COUNT_DURATION * 9) / 8, scale: 1 },
      { x: 0.55, y: 0, delay: (COUNT_DURATION * 5) / 4, scale: 1 },
      { x: 0.69, y: 0, delay: (COUNT_DURATION * 11) / 8, scale: 1 },
      { x: 0.83, y: 0, delay: (COUNT_DURATION * 3) / 2, scale: 1 },
      { x: 0.22, y: 0, delay: (COUNT_DURATION * 13) / 8, scale: 1 },
      { x: 0.5, y: 0, delay: (COUNT_DURATION * 7) / 4, scale: 1 },
      { x: 0.77, y: 0, delay: (COUNT_DURATION * 15) / 8, scale: 1 },
    ],
  },
  {
    count: COUNTS_PER_TWO_EIGHTS,
    mangoCount: 1,
    direction: "left",
    layout: [{ x: 1, y: 0.62, delay: 0, scale: 1 }],
    flipAtEnd: true,
  },
  {
    count: COUNTS_PER_TWO_EIGHTS,
    mangoCount: 1,
    direction: "right",
    layout: [{ x: 0, y: 0.62, delay: 0, scale: 1 }],
    flipped: true,
    flipAtEnd: true,
  },
  {
...
const TOTAL_DURATION = MANGO_SEQUENCE.reduce(
  (total, phase) => total + phase.count * COUNT_DURATION,
  0,
);
...
export function useMangoRun() {
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    const expectedKey = EASTER_EGG_COMMAND[commandProgressRef.current];
    ...
    if (e.key === expectedKey) {
      ...
      if (commandProgressRef.current === EASTER_EGG_COMMAND.length) {
    ...
    commandProgressRef.current = e.key === EASTER_EGG_COMMAND[0] ? 1 : 0;
  }, []);
...
function getMangoStates(
  ...
  const horizontalRight = isFinalExit
    ? viewportInlineSize + mangoSize
    : isTwoEightPhase
      ? viewportInlineSize - mangoSize
      : viewportInlineSize + mangoSize;
  const start =
    phase.direction === "left"
      ? horizontalRight
      : phase.direction === "right"
        ? horizontalLeft
        : phase.direction === "up"
          ? viewportBlockSize + mangoSize
          : -mangoSize;
  const end =
    phase.direction === "left"
      ? horizontalLeft
  ...
  const freezeFrame =
    Math.floor((phaseStart + phase.count * COUNT_DURATION * freezeProgress) / FRAME_DURATION) %
    MANGO_FRAME_PATHS.length;
  const frame = isFrozen
    ? freezeFrame
    : Math.floor(elapsed / FRAME_DURATION) % MANGO_FRAME_PATHS.length;

  return phase.layout.slice(0, phase.mangoCount).map((layout) => {
    const delayedProgress = Math.min(
      Math.max((phaseElapsed - layout.delay) / (phase.count * COUNT_DURATION), 0),
      1,
    );
    ...
    const y = isVertical
      ...
      : isFrozen
        ? frozenY
        : frozenY + Math.sin(elapsed / 80 + layout.x * 10) * 7;
    return {
      ...layout,
      x,
      y,
      ...
      flipped:
        phase.flipAtEnd === true && progress >= flipProgress
          ? !phase.flipped
          : phase.flipped === true,
      frame,
      frozen: isFrozen,
...
const MangoRunEasterEgg = forwardRef<MangoRunEasterEggHandle, MangoRunEasterEggProps>(
  function MangoRunEasterEgg({ onComplete }, ref) {
      ...
      {mangoes.map((mango, index) => (
        ...
        >
          <div className="mango-sprite" style={{ transform: `scaleX(${mango.flipped ? -1 : 1})` }}>
            <img
              src={MANGO_FRAME_PATHS[mango.frame]}
              alt=""
              className={`mango-sprite__image${mango.frozen ? " is-frozen" : ""}`}
            />
          </div>
        </div>
      ))}
